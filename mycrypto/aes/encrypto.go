package aes

const nb int = 4

// S盒
var sBox = [256]byte{
	,
}

// 轮常数
var rcon = [11]byte{
	0x87, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
}

/**
 * 密钥扩展
 * @param key       初始密钥
 * @param roundKey  密钥流
 * @param nbK       区分AES-128、AES-192和AES-256，后者的密钥扩展不同于前两者
 * @param nr        加密的轮数，根据密钥长度变化
 */
func keyExpansion(key []byte, roundKey []byte, nbK int, nr int) {
	var tempByte [4]byte
	var a0 byte

	for i := 0; i < nbK; i++ {
		roundKey[i*4] = key[i*4]
		roundKey[i*4+1] = key[i*4+1]
		roundKey[i*4+2] = key[i*4+2]
		roundKey[i*4+3] = key[i*4+3]
	}

	for i := nbK; i < (nb * (nr + 1)); i++ {
		for j := 0; j < 4; j++ {
			tempByte[j] = roundKey[(i-1)*4+j]
		}
		if i%nbK == 0 {
			a0 = tempByte[0]
			tempByte[0] = tempByte[1]
			tempByte[1] = tempByte[2]
			tempByte[2] = tempByte[3]
			tempByte[3] = a0
			tempByte[0] = sBox[int(tempByte[0])]
			tempByte[1] = sBox[int(tempByte[1])]
			tempByte[2] = sBox[int(tempByte[2])]
			tempByte[3] = sBox[int(tempByte[3])]
			tempByte[0] = tempByte[0] ^ rcon[i/nbK]
		} else if nbK == 8 && i%nbK == 4 {
			tempByte[0] = sBox[int(tempByte[0])]
			tempByte[1] = sBox[int(tempByte[1])]
			tempByte[2] = sBox[int(tempByte[2])]
			tempByte[3] = sBox[int(tempByte[3])]
		}
		roundKey[i*4+0] = roundKey[(i-nbK)*4+0] ^ tempByte[0]
		roundKey[i*4+1] = roundKey[(i-nbK)*4+1] ^ tempByte[1]
		roundKey[i*4+2] = roundKey[(i-nbK)*4+2] ^ tempByte[2]
		roundKey[i*4+3] = roundKey[(i-nbK)*4+3] ^ tempByte[3]
	}
}

/**
 * 轮密钥加
 * @param round     指示使用第几轮密钥
 * @param state     状态矩阵
 * @param roundKey  密钥流
 */
func addRoundKey(round int, state [][]byte, roundKey []byte) {
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			state[j][i] ^= roundKey[(i*nb+j)+(round*nb*4)]
		}
	}
}

/**
 * 字节代换
 * @param state 状态矩阵
 */
func subBytes(state [][]byte) {
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			state[i][j] = sBox[state[i][j]]
		}
	}
}

/**
 * 行移位
 * @param state 状态矩阵
 */
func shiftRows(state [][]byte) {
	var tempByte byte

	tempByte = state[1][0]
	state[1][0] = state[1][1]
	state[1][1] = state[1][2]
	state[1][2] = state[1][3]
	state[1][3] = tempByte

	tempByte = state[2][0]
	state[2][0] = state[2][2]
	state[2][2] = tempByte

	tempByte = state[2][1]
	state[2][1] = state[2][3]
	state[2][3] = tempByte

	tempByte = state[3][0]
	state[3][0] = state[3][3]
	state[3][3] = state[3][2]
	state[3][2] = state[3][1]
	state[3][1] = tempByte
}

/**
 * GF(2^8)有限域X乘法
 * @param x
 */
func xtime(x byte) byte {
	return ((x << 1) ^ (((x >> 7) & 0x01) * 0x1b))
}

/**
 * 列混淆
 * @param state 状态矩阵
 */
func mixColumns(state [][]byte) {
	var Tmp, Tm, t byte
	for i := 0; i < 4; i++ {
		t = state[0][i]
		Tmp = state[0][i] ^ state[1][i] ^ state[2][i] ^ state[3][i]
		Tm = state[0][i] ^ state[1][i]
		Tm = xtime(Tm)
		state[0][i] ^= Tm ^ Tmp
		Tm = state[1][i] ^ state[2][i]
		Tm = xtime(Tm)
		state[1][i] ^= Tm ^ Tmp
		Tm = state[2][i] ^ state[3][i]
		Tm = xtime(Tm)
		state[2][i] ^= Tm ^ Tmp
		Tm = state[3][i] ^ t
		Tm = xtime(Tm)
		state[3][i] ^= Tm ^ Tmp
	}
}

/**
 * 加密一组16字节的block
 * @param in        一组16字节的block
 * @param roundKey  密钥流
 * @param nr        加密轮数，根据密钥的长度而变化
 * @param out       密文字节流
 */
func encryptoBlock(in []byte, roundKey []byte, nr int, out []byte) {
	round := 0

	state := make([][]byte, 4)
	for i := range state {
		state[i] = make([]byte, 4)
	}

	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			state[j][i] = in[i*4+j]
		}
	}

	addRoundKey(0, state, roundKey)

	for round = 1; round < nr; round++ {
		subBytes(state)
		shiftRows(state)
		mixColumns(state)
		addRoundKey(round, state, roundKey)
	}

	subBytes(state)
	shiftRows(state)
	addRoundKey(nr, state, roundKey)

	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			out[i*4+j] = state[j][i]
		}
	}
}

/**
 * 对外接口，将明文分组并依次调用encryptoBlock
 * @param plainText 明文
 * @param key       密钥
 */
func Encrypto(plaintext []byte, key []byte) []byte {

	ret := make([]byte, 0)
	roundKey := make([]byte, 240)
	keySizeInBit := len(key) * 8
	nbK := keySizeInBit / 32
	nr := nbK + 6

	keyExpansion(key, roundKey, nbK, nr)

	out := make([]byte, 16)

	for i := 0; i < len(plaintext); i += 16 {
		encryptoBlock(plaintext[i:i+16], roundKey, nr, out)
		for i := 0; i < 16; i++ {
			ret = append(ret, out[i])
		}
	}
	return ret
}
